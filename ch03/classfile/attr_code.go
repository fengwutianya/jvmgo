...
/*
Code_attribute {
    u2 attribute_name_index;		//attribute的name存在于constant pool，这里是个index
    u4 attribute_length;			//属性的length
    u2 max_stack;					//操作数栈的最大深度，运行时数据区会用到
    u2 max_locals;					//局部变量表的大小，运行时数据区
    u4 code_length;					//字节码区长度
    u1 code[code_length];			//所有的字节码
    u2 exception_table_length;		//异常表的长度
    {   u2 start_pc;
        u2 end_pc;
        u2 handler_pc;
        u2 catch_type;
    } exception_table[exception_table_length];	//异常们
    u2 attributes_count;						//属性表的长度
    attribute_info attributes[attributes_count];//属性们
}
*/
